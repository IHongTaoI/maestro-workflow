...
Activity Timeline 从带有可靠 `completed_at` 的 Task，以及带有 `decided_at` 的里程碑级不可变
Decision Record 与 Playbook 评审记录（`playbooks/decisions/`）派生，回答“某段时间完成了什么、
做过哪些关键决定、批准了哪些 Playbook”。它只生成本地可重建的
