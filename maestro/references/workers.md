...
位于选定项目的 `.maestro/workers/registry.yaml`。因此，新项目起初没有执行 Worker，只能通过
明确的项目注册表变更逐步增加。
...
校验空的内置注册表协议；如果存在，则加载 `.maestro/workers/registry.yaml`。只有项目注册表
...
只有检查、非破坏性验证、Maestro 状态维护和有界工作工件写入可以声明为 autonomous。编辑项目
