...
Playbook 是项目自行编写的可选指导，使用 Markdown 或 YAML 存储在 `.maestro/playbooks/` 下。
...
受管理的正式 Playbook 是 `.maestro/playbooks/` 下的单个 Markdown 或 YAML 文件，不包括保留
...
请求校验仅在 `current_playbooks.file_path` 指向 `.maestro/playbooks/` 下已存在的 Markdown
