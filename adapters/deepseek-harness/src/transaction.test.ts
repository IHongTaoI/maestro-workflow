...
function input(): TransactionInput {
  return {
    ...
    members: [
      { kind: 'create', path: '.maestro/tasks/new/progress.md', staged: 'new' },
      { kind: 'replace', path: '.maestro/memory/current.md', staged: 'after', base_hash: '' },
    ],
  }
}
...
test('committed interruption exposes overlay and recover materializes idempotently', async () => {
  ...
  assert.deepEqual(await tx.readOverlay('.maestro/memory/current.md'), {
...
test('missing applied record is repaired without rewriting staged canonical bytes', async () => {
  ...
  const recovered = await tx.recover('tx-1', { actor: 'recovery-agent' })
  assert.equal(recovered.status === 'committed' && recovered.materialization, 'complete')
  assert.equal(f.files.get('.maestro/tasks/new/progress.md')?.version, versionBefore)
})
...
test('locks are acquired in stable member order and released as tombstones', async () => {
  ...
  assert.deepEqual(held, [
    '.maestro/locks/memory-current.md.lock',
    '.maestro/locks/tasks-new-progress.md.lock',
    '.maestro/locks/transactions-tx-1-terminal.lock',
  ])
...
test('missing staged members and forged persisted paths fail closed', async () => {
  ...
  const unexpected = memoryFs(), unexpectedTx = new MaestroTransactionStore(unexpected.fs)
  unexpected.put('.maestro/transactions/not-a-bundle.txt', 'unexpected')
  await assert.rejects(() => unexpectedTx.readOverlay('.maestro/memory/current.md'),
...
test('member and transaction byte limits fail before transaction records are written', async () => {
  const oversized = memoryFs(), oversizedTx = new MaestroTransactionStore(oversized.fs)
  ...
  assert.equal([...total.files.keys()].some((path) => path.startsWith('.maestro/transactions/')), false)
  ...
  assert.equal([...before.files.keys()].some((path) => path.startsWith('.maestro/transactions/')), false)
})
...
test('a completed historical transaction never overlays or overwrites a later legal transaction', async () => {
  ...
  assert.equal(f.files.get('.maestro/memory/current.md')?.content, 'later')
  assert.deepEqual(await tx.readOverlay('.maestro/memory/current.md'), { source: 'canonical', content: 'later' })
})
