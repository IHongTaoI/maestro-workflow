...
11. 提升 Temporary 后，Task 元数据里的 `source_temporary` 与 `promotion_transaction` 让晋升事件自动
    出现在提交标记记录的时刻；`--event-type temporary_promoted` 能单独筛出该事件，同一个 Task 的
    完成事件仍然独立存在。
12. 缺少提交标记、标记格式未知，以及 `preparing` 状态的历史 Task 都不产生晋升事件；不得用
    `created_at`、`updated_at`、mtime 或 Git 时间当作晋升时间。
13. 提交标记形状正确但编码了不可能的时刻时，构建明确失败；Task 移入 `tasks/archive/` 后晋升事件
    的事件 ID 与发生时间不变，引用指向当前文件。
