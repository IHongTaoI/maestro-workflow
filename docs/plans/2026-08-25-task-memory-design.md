...
.maestro/
  ...
  tasks/<task-id>/runs/<run-id>.result.json
...
Graph digest, active run identifier and timestamps. `run-000001.json` is the immutable prepared
request; `run-000001.result.json` contains the immutable validated result and Artifact identifiers. A task has one active
...
DSH request. `record-task-run` validates the returned JSON, snapshots declared artifacts, writes a
separate result receipt, derives a memory entry, then marks the task `completed` when every child has no
