...
```powershell
npm run maestro -- compile-task-graph --file <task-graph.yaml>
```
...
若当前项目还没有安装此 Skill，先由用户在项目根目录执行 `npm run maestro -- install-dsh-skill`；执行 `npm run maestro -- verify-dsh-skill` 可以校验安装副本仍和仓库内置版本一致。安装或编译成功不代表 Workflow 已经执行。
