...
test("guards role effects through the five Core actions", async () => {
  ...
  try {
    ...
    await writeFile(join(permissionDirectory, "permission-legacy.json"), `${JSON.stringify({ schemaVersion: 1, id: "permission-legacy", workspaceId: "core-probe", role: "coder", read: [], write: [".agents"], execute: [], appliedAt: "2026-08-26T00:00:00.000Z" }, null, 2)}\n`);
    assert.equal((await validateProposal({ projectRoot: root, workspaceId: "core-probe", proposalId: protectedProposal.id, permissionGrantId: "permission-legacy" })).status, "rejected");
