...
test('a promoted Task becomes a promotion event at its commit marker time', async (t) => {
  const projectRoot = await createProject(t);
  await seedTask(projectRoot, {
    id: 'promoted',
    objective: '优化登录流程',
  ...
  assert.equal(event.occurred_at, '2026-08-31T12:00:00Z');
...
test('promotion markers without a commit time never invent an event', async (t) => {
  const projectRoot = await createProject(t);
  await seedTask(projectRoot, {
    id: 'unknown-marker',
    objective: '旧格式标记',
    ...
    promotionTransaction: 'arbitrary-unknown-marker',
  });
  await seedTask(projectRoot, {
    id: 'source-only',
    objective: '只有来源没有标记',
  ...
  assert.ok(result.events.every((event) => !event.summary.includes('晋升')));
});

test('a promotion marker encoding an impossible instant fails the build', async (t) => {
  const projectRoot = await createProject(t);
  await seedTask(projectRoot, {
    id: 'bad-marker',
    objective: '时间非法',
    ...
    completedAt: null,
    sourceTemporary: '20260901-主题',
    promotionTransaction: '20261399T120000Z-abcdef12',
  });

  const error = await rejectedCommand(runActivity(projectRoot, ['build']));
  assert.match(error.stderr, /invalid commit time/);
});
...
test('moving a promoted Task to archive keeps its promotion event stable', async (t) => {
  ...
  assert.equal(after.events[0].occurred_at, '2026-08-31T12:00:00Z');
